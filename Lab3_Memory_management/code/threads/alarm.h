...
class Alarm : public CallBackObj {
  ...
  private:
    ...
    public:
        ...
        bool checkWakeup();
        void addThread(Thread* t, int x) { sl.Insert(sleepThread(t, counter + x)); }
    private:
        typedef pair<Thread*, int> sleepThread;

        int counter;
        SortedList<sleepThread> sl;

        static int cmp(sleepThread t1, sleepThread t2) { return t1.second < t2.second; }
    } sleepList;
